...
void walls(glm:: mat4 initMatrix){
    ...
    glUniform4f(spSimplest->u("kd"), 0.2f, 0.0f, 0.5f, 1.0f);   //0.2f, 0.6f, 0.4f, 1.0f
    ...
    glUniform4f(spSimplest->u("kd"), 0.2f, 0.0f, 0.5f, 1.0f);
    ...
    mat4 ceilingMatrix = translate(initMatrix, vec3(0, 10, 0));
    mat4 scaledCeilingMatrix4 = scale(ceilingMatrix, vec3(C_ROOM_SIZE, 0.1f, C_ROOM_SIZE));
    glUniform4f(spSimplest->u("kd"), 0.7f, 0.7f, 0.7f, 1.0f);
    glUniformMatrix4fv(spSimplest->u("M"), 1, false, value_ptr(scaledCeilingMatrix4));
    Models::cube.drawSolid();
...
glm::mat4 aquariumDraw(glm::mat4 matrix) {
    ...
    for (int i = aquariumModel.size() - 1; i >= 0; i--) {
        ...
        glUniform1f(shader->u("ambient"), 0.2f);
